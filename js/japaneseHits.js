...
const japaneseData = hitsResponse.data.data[2].id;
//取得歌單各別歌曲
function getTracks(id){
    axios.get(`https://api.kkbox.com/v1.1/new-hits-playlists/${id}/tracks?territory=TW&limit=30`,kkboxConfig).then((res) => {
        const japaneseHitsSong = res.data.data
        hitsSongsPlay(japaneseHitsSong)
    })
}

getTracks(japaneseData)
...
function hitsSongsPlay(hitsData){
    let japaneseHitspalyer = ""
    hitsData.forEach((item,index) => {
        japaneseHitspalyer+=`<li data-songId="${item.id}" class="list-group-item
    ...
    hitssSongsList.innerHTML = japaneseHitspalyer;
};
//YT影片
...
playBtns.addEventListener("click",(e) =>{
    if(e.target.dataset.start === "start"){
        ...
        src="https://widget.kkbox.com/v1/?id=${japaneseData}&type=playlist"
