...
const session3Data = sessionResponse.data.data[2].id;
const session3title = sessionResponse.data.data[2].title
...
function getTracks(id){
    axios.get(`https://api.kkbox.com/v1.1/new-hits-playlists/${id}/tracks?territory=TW&limit=30`,kkboxConfig).then((res) => {
        const session3Song = res.data.data
        hitsSongsPlay(session3Song)
        title.textContent = session3title
    })
}

getTracks(session3Data)
...
function hitsSongsPlay(hitsData){
    ...
    hitsData.forEach((item,index) => {
        session3spalyer+=`<li data-songId="${item.id}" class="list-group-item
    ...
    sessionSongsList.innerHTML = session3spalyer;
};
//YT影片
...
playBtns.addEventListener("click",(e) =>{
    if(e.target.dataset.start === "start"){
        ...
        src="https://widget.kkbox.com/v1/?id=${session3Data}&type=playlist"
