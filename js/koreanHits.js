...
const koreanData = hitsResponse.data.data[3].id;
//取得歌單各別歌曲
function getTracks(id){
    axios.get(`https://api.kkbox.com/v1.1/new-hits-playlists/${id}/tracks?territory=TW&limit=30`,kkboxConfig).then((res) => {
        const koreanHitsSong = res.data.data
        hitsSongsPlay(koreanHitsSong)
    })
}

getTracks(koreanData)
...
function hitsSongsPlay(hitsData){
    let koreanHitspalyer = ""
    hitsData.forEach((item,index) => {
        koreanHitspalyer+=`<li data-songId="${item.id}" class="list-group-item
    ...
    hitssSongsList.innerHTML = koreanHitspalyer;
};
//YT影片
...
playBtns.addEventListener("click",(e) =>{
    if(e.target.dataset.start === "start"){
        ...
        src="https://widget.kkbox.com/v1/?id=${koreanData}&type=playlist"
