...
const session4Data = sessionResponse.data.data[3].id;
const session4title = sessionResponse.data.data[3].title
...
function getTracks(id){
    axios.get(`https://api.kkbox.com/v1.1/new-hits-playlists/${id}/tracks?territory=TW&limit=30`,kkboxConfig).then((res) => {
        const session4Song = res.data.data
        hitsSongsPlay(session4Song)
        title.textContent = session4title
    })
}

getTracks(session4Data)
...
function hitsSongsPlay(hitsData){
    ...
    hitsData.forEach((item,index) => {
        session4spalyer+=`<li data-songId="${item.id}" class="list-group-item
    ...
    sessionSongsList.innerHTML = session4spalyer;
};
//YT影片
...
playBtns.addEventListener("click",(e) =>{
    if(e.target.dataset.start === "start"){
        ...
        src="https://widget.kkbox.com/v1/?id=${session4Data}&type=playlist"
