...
const chineseData = hitsResponse.data.data[0].id;
//取得歌單各別歌曲
function getTracks(id){
    axios.get(`https://api.kkbox.com/v1.1/new-hits-playlists/${id}/tracks?territory=TW&limit=30`,kkboxConfig).then((res) => {
        const chineseHitsSong = res.data.data
        hitsSongsPlay(chineseHitsSong)
    })
}

getTracks(chineseData)
...
function hitsSongsPlay(hitsData){
    let chineseHitspalyer = ""
    hitsData.forEach((item,index) => {
        chineseHitspalyer+=`<li data-songId="${item.id}" class="list-group-item
    ...
    hitssSongsList.innerHTML = chineseHitspalyer;
};
//YT影片
...
playBtns.addEventListener("click",(e) =>{
    if(e.target.dataset.start === "start"){
        ...
        src="https://widget.kkbox.com/v1/?id=${chineseData}&type=playlist"
